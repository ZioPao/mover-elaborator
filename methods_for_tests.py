...
def print_validation_curves(clf, title, X_test, y_test, param, param_range):
...
def print_learning_curves(clf_1, title_1, clf_2, title_2, X_test, y_test, cv):
    fig, axes = plt.subplots(3, 2, figsize=(10, 15))
    plot_learning_curve(clf_1, title_1, X_test, y_test, axes=axes[:, 0], ylim=(0.5, 1.01), cv=cv, n_jobs=-1)
    plot_learning_curve(clf_2, title_2, X_test, y_test, axes=axes[:, 1], ylim=(0.5, 1.01), cv=cv, n_jobs=-1)
    plt.show()
